...
function applyLinuxChromePluginAutoInstallPatch(currentSource) {
  ...
  const spreadChromeGateRegex =
    /\{\.\.\.([A-Za-z_$][\w$]*\.[A-Za-z_$][\w$]*\.chrome),((?!installWhenMissing:!0,)[^{}]*?isAvailable:\(\{buildFlavor:[A-Za-z_$][\w$]*,features:[A-Za-z_$][\w$]*\}\)=>[^{}]*?externalBrowserUseAllowed[^{}]*?)\}/g;
  let spreadChromeGatePatched = false;
  const spreadChromePatchedSource = currentSource.replace(
    spreadChromeGateRegex,
    (_match, descriptorExpr, remainingFields) => {
      spreadChromeGatePatched = true;
      return `{...${descriptorExpr},installWhenMissing:!0,${remainingFields}}`;
    },
  );
  if (spreadChromeGatePatched) {
    return spreadChromePatchedSource;
  }
  if (
    /\{\.\.\.[A-Za-z_$][\w$]*\.[A-Za-z_$][\w$]*\.chrome,installWhenMissing:!0,[^{}]*?isAvailable:\(\{buildFlavor:[A-Za-z_$][\w$]*,features:[A-Za-z_$][\w$]*\}\)=>[^{}]*?externalBrowserUseAllowed[^{}]*?\}/.test(currentSource)
  ) {
    return currentSource;
  }
