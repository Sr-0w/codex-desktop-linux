...
UPSTREAM_DMG_URL="${UPSTREAM_DMG_URL:-}"
...
if [ ! -s "$UPSTREAM_DMG_PATH" ]; then
    if [ -z "$UPSTREAM_DMG_URL" ]; then
        UPSTREAM_DMG_URL="$(read_nix_fetchurl_field codexDmg url)"
    fi
...
dmg_parcel_watcher_version="$(json_file_field "$ASAR_EXTRACT_DIR/package.json" "value.dependencies['@parcel/watcher']")"
